...
export class AuthenticationService {
		...
		try {
			...
			if (user)
				throw new BadRequestException(
					this.errorMessagesService.EMAIL_EXISTS,
				);
	...
	async loginLocal(
		...
		try {
			...
			if (!user)
				throw new BadRequestException(
					this.errorMessagesService.INVALID_CREDENTIALS_PROVIDED,
				);
		...
		try {
			...
			if (!user)
				throw new ForbiddenException(
					this.errorMessagesService.FORBIDDEN,
				);
